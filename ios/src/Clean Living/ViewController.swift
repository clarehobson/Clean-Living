...
// IAP MANAGER
// =====================
let iapMonthlyId = "app.cleanliving.pro.monthly2"
let iapAnnualId  = "app.cleanliving.pro.annual2"
...
class IAPManager: NSObject, SKProductsRequestDelegate, SKPaymentTransactionObserver {
    ...
    func productsRequest(_ request: SKProductsRequest, didReceive response: SKProductsResponse) {
        ...
        sendProductsToWeb()
        // If there was a pending purchase, attempt it now
        if let pending = pendingProductId {
            pendingProductId = nil
            if let product = products.first(where: { $0.productIdentifier == pending }) {
                let payment = SKPayment(product: product)
                SKPaymentQueue.default().add(payment)
            } else {
                sendResultToWeb(success: false, productId: pending, error: "Product not found — please try again")
            }
        }
    }
    ...
    func purchase(productId: String) {
        ...
        if let product = products.first(where: { $0.productIdentifier == productId }) {
            ...
            // Products not loaded yet — store pending and fetch
...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    var pendingDeepLink: [String: Any]? = nil
    ...
    override func viewDidLoad() {
        ...
        OneSignal.initialize("7976d6f8-e7ae-4424-a683-3b7e43fcc7ab", withLaunchOptions: nil)
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        // Fire any pending deep link from a notification tap
        if let deepLink = pendingDeepLink {
            pendingDeepLink = nil
            if let jsonData = try? JSONSerialization.data(withJSONObject: deepLink),
...
extension ViewController: WKScriptMessageHandler {
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        } else if message.name == "onesignal-login" {
            // Link OneSignal subscription to the logged-in user's Supabase UUID
